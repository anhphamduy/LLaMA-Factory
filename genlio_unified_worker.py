#!/usr/bin/env python3
"""
Unified LLaMA-Factory Worker for Genlio Integration
...
Usage:
    python genlio_unified_worker.py --database-url "postgresql://..." --poll-interval 5
...
logger = logging.getLogger("genlio_unified_worker")
...
class UnifiedWorker:
    ...
    def __init__(self, config: WorkerConfig):
        ...
        # Single status reporter for unified worker
        ...
        logger.info(f"Unified Worker {config.worker_id} initialized")
    ...
    def run(self):
        """Main worker loop."""
        logger.info(f"Unified Worker {self.config.worker_id} starting main loop")
        ...
        logger.info("Unified Worker shutting down")
...
def main():
    parser = argparse.ArgumentParser(description="Genlio Unified LLaMA-Factory Worker")
    parser.add_argument("--database-url", default=os.environ.get("DATABASE_URL"))
    parser.add_argument("--worker-id", default=os.environ.get("WORKER_ID", f"unified-{uuid.uuid4().hex[:8]}"))
